                    ...
                    if (entity instanceof Animals || entity.getType().equals(EntityType.SQUID)) {
                        ...
                        for (ItemStack itemInHand: Util.getPlayerInHandItems((Player) entity)) {
                            if (itemInHand != null) {
                                ...
                                if (type == Material.EGG || type == Material.LEGACY_MONSTER_EGG || type == Material.WHEAT || type == Material.LEGACY_CARROT_ITEM
                                        || type == Material.LEGACY_SEEDS) {
                                    if (DEBUG2)
        ...
        if (IslandGuard.inWorld(e.getPlayer())) {
            ...
            if (island.getIgsFlag(SettingsFlag.PLACE_BLOCKS) || island.getMembers().contains(e.getPlayer().getUniqueId()))  {
                ...
                if (!e.getBlock().getState().getClass().getName().endsWith("CraftBlockState")
                        ...
                        || e.getBlock().getType().equals(Material.LEGACY_REDSTONE_COMPARATOR_OFF)
                        || type.endsWith("BANNER") // Avoids V1.7 issues
                        || e.getBlock().getType().equals(Material.ENDER_CHEST)
                        || e.getBlock().getType().equals(Material.LEGACY_ENCHANTMENT_TABLE)
    ...
    public void onTreeGrow(final StructureGrowEvent e) {
        ...
        while (it.hasNext()) {
            ...
            if (b.getType() == Material.LEGACY_LOG || b.getType() == Material.LEGACY_LOG_2
                    || b.getType() == Material.LEGACY_LEAVES || b.getType() == Material.LEGACY_LEAVES_2) {
